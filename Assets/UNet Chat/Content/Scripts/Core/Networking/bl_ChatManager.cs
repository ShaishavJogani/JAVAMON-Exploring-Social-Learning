...
public class bl_ChatManager : NetworkBehaviour
{
    ...
    public void FindAll() {
      ...
      objectName = GameObject.Find("GameObjectId").GetComponent<UnityEngine.UI.Text>();
    }
    ...
    /// <param name="t"></param>
    /// <param name="id"></param>
    public void SendMessageChat(string t, int id)
    {
    ...
    public void OnChatMessage(NetworkMessage netMsg)
    {
        ...
        else if (Client.isConnected)//if this client
        {
            ...
            if(t.Contains("vuforia")) {
              string[] newt = t.Split(new string[] { ":" }, StringSplitOptions.None);
              string character = newt[2];
              objectName.text = character;
              // if(character == "Charmander") {
              //   Debug.Log("Charmander is clicked");
              //   c1.SetActive(false);
        		// c2.SetActive(true);
        		// c3.SetActive(false);
              // } else if(character == "Charmeleon") {
              //   Debug.Log("Charmeleon is clicked");
              //   c2.SetActive(false);
              //   c1.SetActive(false);
        		// c3.SetActive(true);
              // } else if(character == "Charizard") {
              //   Debug.Log("Charlli is clicked");
              //   c1.SetActive(true);
        		// c2.SetActive(false);
        		// c3.SetActive(false);
              // }
    ...
    public string GetMessageFormat(string msn, string sender, bl_GroupInfo group)
    {
        ...
        if (true)
        {
            m = string.Format("<color=#{0}>{2}: </color>{1}", hex, filterText, sender);
        }
        else
        {
